...
import { earnPointsAction } from "@/app/dashboard/[businessId]/loyalty/actions";
...
async function earnPointsWithReset(
  prev: State,
  formData: FormData
): Promise<State> {
  ...
  try {
    ...
    // earnPointsAction always redirects on success; this line is unreachable.
...
export function EarnPointsForm({ businessId, memberships, csrfField }: Props) {
  ...
  return (
    <form action={formAction} className="grid gap-3">
      ...
      <SubmitButton variant="primary">Issue points</SubmitButton>
    </form>
  );
}
