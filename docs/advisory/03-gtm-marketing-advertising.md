...
3. ~~Build business-to-business referrals.~~ **Shipped** — Rand account-credit loop (`merchant-referral.service` + `merchant-credit.service`, dashboard at `settings/referrals`, share link `/onboard?mref=CODE`). Remaining: prove conversion in-market, make Stripe/PayFast credit-aware (currently Yoco only), and report referral-sourced CAC in the cohort dashboard.
