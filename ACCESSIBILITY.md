...
- **Vitest + axe-core**: smoke tests assert that the Button, Input, and Card primitives produce no critical axe violations. Mutation surfaces also carry unit coverage for live-region status announcements. Run with `npm test`.
...
- Extend axe smoke coverage from primitives into the full auth and dashboard mutation flows.
- Broaden ARIA-live coverage from the current profile/push/document/auth status regions into dashboard loyalty mutations and staff messaging.
